...
private: 
    ...
    vector<long long> build(int u, int l, int r) { 
        if (l == r) { 
            // Si n = 1 (u = 1), guardar rootY antes de retornar
            if (u == 1) {
                rootY = {pts[l].y};
            }
            return {pts[l].y}; 
...
public: 
    ...
    int query(long long x1, long long x2, long long y1, long long y2) const { 
        ...
        auto compLower = [](const Point& p, long long val) { return p.x < val; }; 
        auto compUpper = [](long long val, const Point& p) { return val < p.x; }; 

        int ql = lower_bound(pts.begin(), pts.end(), x1, compLower) - pts.begin(); 
        int qr = upper_bound(pts.begin(), pts.end(), x2, compUpper) - pts.begin() - 1; 
